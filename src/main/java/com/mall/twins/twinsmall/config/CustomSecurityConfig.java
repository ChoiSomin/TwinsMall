...
@EnableGlobalMethodSecurity(prePostEnabled = true)  // 특정 경로에 접근할 수 있는 권한 설정
public class CustomSecurityConfig {

    private final DataSource dataSource;
    private final CustomUserDetailsService userDetailsService;

    // 패스워드 암호화 (필수) -> CustomUserDetailsService 정상 작동 하기 위해 필요함
    @Bean
    ...
    public AccessDeniedHandler accessDeniedHandler() {
        //accessDeniedHandler 메소드가 반영되면 403 메세지가 출력되는 대신 로그인 페이지로 이동
        //error=ACCESS_DENIED 파라미터 값이 전달됨
